...
export function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!password || !confirmPassword) {
      toast.error('Both fields are required!', { position: 'top-right', autoClose: 5000 });
      return;
    }
    ...
    if (!token) {
      toast.error('Invalid or missing token.', { position: 'top-right', autoClose: 5000 });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(responseData.message || 'Failed to reset password', { position: 'top-right', autoClose: 5000 });
        return;
      }
      

      toast.success('Password has been reset successfully!', { position: 'top-right', autoClose: 5000 });
